...
local function handle_stream(chunk, provider)
    ...
    local handlers = {
        ...
        on_complete = function(err)
            if err then
                log.error("Stream error: " .. vim.inspect(err), { notify = true })
                return
            end
...
function M.send_message(content, opts)
    ...
    local job = Job:new({
        ...
        args = {
            ...
            '-H', 'Authorization: Bearer ' .. provider.parse_api_key(),
            '--data-raw', vim.json.encode(curl_args.body),
            curl_args.url
        },
...
%s
    ...
    local curl_args = provider.parse_curl_args(provider, request_opts)
