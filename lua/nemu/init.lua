...
function M.setup(user_config)
	...
	vim.api.nvim_create_user_command("NemuPanel", function()
		panel.toggle()
	end, { desc = "Toggle the Nemu assistant side panel." })

	vim.api.nvim_create_user_command("NemuSlash", function()
		commands.open_slash_picker()
	end, { desc = "Open the slash command picker (Telescope)." })

	vim.api.nvim_create_user_command("NemuAccept", function()
		diff.accept_current()
	end, { desc = "Accept the currently streamed AI diff." })

	vim.api.nvim_create_user_command("NemuReject", function()
		diff.reject_all()
	end, { desc = "Reject the currently streamed AI diff (Not fully implemented yet)." })
